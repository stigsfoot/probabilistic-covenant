...
It proves one thing, using a system I shipped. Copa Chalkboard's validation gate is
pure, deterministic, and unit-tested. Run five times on one still photograph, it
...
That finding is not staged for this talk. It is recorded in the Copa repo at
...
## Run it end to end
    ...
    ollama pull gemma3:4b                    # 2. slow; start early
    ...
    uv run python eval/faithfulness.py       # 6. real grounding scores
...
- `fixtures/analyst_report.json`: PLACEHOLDER. Not on the stage path; `capture.py`
  ...
  five notes before you decide which step wrote them (step 1 below).
...
- `fixtures/judge_scores.json`: PLACEHOLDER. Regenerate with `eval/faithfulness.py`.
  Never show a fabricated number — and the demo now enforces that rather than
  trusting you to remember it. While this file says `placeholder: true`, `ship.py`
  prints "not yet measured" wherever a faithfulness score would go. Real scores
  appear on stage only once the judge has actually produced them.
...
1. Confirm which step wrote the five notes. `analyst.py` states the Analyst never
   sees the image and the `ScoutReport` schema carries no jersey numbers or kit
   colours, yet the notes say "number 11," "white," and "blue." If they came from
   `run_pipeline_local`, the Analyst invented those details from coordinates alone,
   which is the stronger story. If they came from the smoketest's free-form vision
   call, say that instead. One sentence in the talk depends on which.
...
4. Run `faithfulness.py`. Confirm the primary note lands clearly under 0.85 — and
   then read all five scores, not just the primary. Notes 2-5 narrate motion just
   as note 1 does ("drives forward," "chasing," "transition"), so a judge that
   scores them high would put high faithfulness numbers on screen one beat after
   the red banner said motion narration is the failure, contradicting the closing
   line. If the judge does rate them high, either say so out loud as a finding
   about the judge, or drop the per-note numbers from the reveal. Do not let the
   projector argue against you.
...
- **DeepEval** (Confident AI, open source), the pytest-native LLM evaluation framework
  and its FaithfulnessMetric. A failing metric fails the build like any other test.
